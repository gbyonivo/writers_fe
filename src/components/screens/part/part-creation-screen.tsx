...
export function PartCreationScreen() {
  ...
  const joinPreviousPartIds = (localSearchParams.previousPartId || '') as string
  const { parts: allPieceParts, loading, error } = usePieceParts(pieceId)

  const parts = useMemo(() => {
    const previousParts = joinPreviousPartIds.split(',').reduce((acc, curr) => {
      const part = allPieceParts.find((part) => part.id === parseInt(curr, 10))
      if (!part) return acc
      return [...acc, part]
    }, [])
    return [...previousParts, undefined]
  }, [joinPreviousPartIds, allPieceParts])
